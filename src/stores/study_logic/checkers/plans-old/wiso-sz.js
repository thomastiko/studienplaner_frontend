...
async function checkHauptstudium(study, twoLvCheck, ectsCount, data) {
  const update_array = [];
  const hauptstudium = study.subjectStatusEntry.slice(13, 45);
...
async function checkWahlpflichtfach(study, isRunningCheckCBK, twoLvCheck) {
  const update_array = [];
  const spezialisierungsangebot = study.subjectStatusEntry.slice(30, 35);
...
async function checkWahlpflichtfachSoz(study, isRunningCheckCBK, twoLvCheck) {
  const update_array = [];
  const spezialisierung = study.subjectStatusEntry.slice(44, 52);
  console.log(spezialisierung);
  const gruppe1 = spezialisierung.slice(0, 2);
  const gruppe2 = spezialisierung.slice(2, 4);
  const gruppe3 = spezialisierung.slice(4, 6);
  const gruppe4 = spezialisierung.slice(6, 8);

  if (isRunningCheckCBK && twoLvCheck) {
      spezialisierung.forEach((item) => {
        if (item.status == "unavailable") {
          item.status = "can-do";
          update_array.push({
            study_id: study.study_id,
       ...
       if (
      gruppe1.some((item) => item.status == "done" || item.status == "doing") &&
      gruppe2.every((item) => item.status == "can-do") &&
      gruppe3.every((item) => item.status == "can-do") &&
      gruppe4.every((item) => item.status == "can-do")
    )  {
      gruppe2.forEach((item) => {
        item.status = "unavailable";
        update_array.push({
          study_id: study.study_id,
          subject_id: item.subject_id,
      ...
      gruppe1.every((item) => item.status == "can-do") &&
      gruppe2.every((item) => item.status == "can-do" || item.status == "unavailable") &&
      gruppe3.every((item) => item.status == "can-do" || item.status == "unavailable") &&
      gruppe4.every((item) => item.status == "can-do" || item.status == "unavailable")
    ) {
      {
        gruppe2.forEach((item) => {
          item.status = "unavailable";
     ...
     if (
      gruppe2.some((item) => item.status == "done" || item.status == "doing") &&
      gruppe1.every((item) => item.status == "can-do") &&
      gruppe3.every((item) => item.status == "can-do") &&
      gruppe4.every((item) => item.status == "can-do")
    )  {
      {
        gruppe1.forEach((item) => {
          item.status = "unavailable";
          update_array.push({
            study_id: study.study_id,
            subject_id: item.subject_id,
      ...
      gruppe2.every((item) => item.status == "can-do") &&
      gruppe1.every((item) => item.status == "can-do" || item.status == "unavailable") &&
      gruppe3.every((item) => item.status == "can-do" || item.status == "unavailable") &&
      gruppe4.every((item) => item.status == "can-do" || item.status == "unavailable")
    ) {
      {
      ...
      gruppe3.some((item) => item.status == "done" || item.status == "doing") &&
      gruppe1.every((item) => item.status == "can-do") &&
      gruppe2.every((item) => item.status == "can-do") &&
      gruppe4.every((item) => item.status == "can-do")
    )  {
      ...
      gruppe4.some((item) => item.status == "done" || item.status == "doing") &&
      gruppe1.every((item) => item.status == "can-do") &&
      gruppe3.every((item) => item.status == "can-do") &&
      gruppe2.every((item) => item.status == "can-do")
    )  {
...
async executeAll(study, data, isRunningCheckCBK, twoLvCheck, ectsCount) {
 ...
 const wahlpflichtfachSozValues = await checkWahlpflichtfachSoz(study, isRunningCheckCBK, twoLvCheck);
 ...
 update_array.push({wahlpflichtfachSozValues});
