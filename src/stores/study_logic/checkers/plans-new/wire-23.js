...
async function checkOeffRechtMue(study) {
 ...
 async function checkSbwl(study, twoCbkSubjectsDone) {
  const update_array = [];
    ...
    const amc = study.subject_states.find(
      (i) => i._id == "4"
    );
...
async function checkGwaUndWahlfach(study) {
  const update_array = [];
    const glÖffRecht = study.subject_states.find(
      (i) => i._id == "12"
    );
    const glZviliRecht = study.subject_states.find(
      (i) => i._id == "13"
    );
    const gwa = study.subject_states.find(
      (i) => i._id == "22"
    );
    const wahlfach = study.subject_states.find(
      (i) => i._id == "23"
    );
    if (
